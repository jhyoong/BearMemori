...
async def test_create_llm_job_with_user(test_app, test_user):
    ...
    resp = await test_app.post("/llm_jobs", json={
        "job_type": "image_tag",
    ...
    assert data["payload"] == {"memory_id": "mem-123", "image_url": "https://example.com/img.jpg"}
...
async def test_get_llm_job(test_app, test_user):
    ...
    create_resp = await test_app.post("/llm_jobs", json={
        "job_type": "task_match",
...
async def test_update_llm_job_status(test_app, test_user):
    ...
    patch_resp = await test_app.patch(f"/llm_jobs/{job_id}", json={
        "status": "processing",
    })
    assert patch_resp.status_code == 200
    assert patch_resp.json()["status"] == "processing"

    patch_resp2 = await test_app.patch(f"/llm_jobs/{job_id}", json={
        "status": "completed",
    })
...
async def test_update_llm_job_404(test_app, test_user):
    ...
    resp = await test_app.patch("/llm_jobs/nonexistent-id-99999", json={
        "status": "completed",
    })
    assert resp.status_code == 404
...
async def test_update_llm_job_audit_log(test_app, test_user, test_db):
    ...
    await test_app.patch(f"/llm_jobs/{job_id}", json={
        "status": "completed",
    })
...
async def test_list_llm_jobs_no_filter(test_app, test_user):
    ...
    for payload in [
        {"job_type": "image_tag", "payload": {"memory_id": "mem-1"}, "user_id": test_user},
        {"job_type": "intent_classify", "payload": {"message": "Hello"}, "user_id": test_user},
        {"job_type": "followup", "payload": {"context": "Previous chat"}, "user_id": test_user},
    ]:
...
async def test_list_llm_jobs_filter_status(test_app, test_user):
    ...
    for jt in ["image_tag", "intent_classify", "followup"]:
        r = await test_app.post("/llm_jobs", json={
            "job_type": jt,
            "payload": {"key": jt},
            "user_id": test_user,
        })
...
async def test_list_llm_jobs_filter_job_type(test_app, test_user):
    ...
    await test_app.post("/llm_jobs", json={
        "job_type": "image_tag", "payload": {"m": "1"}, "user_id": test_user,
    })
    await test_app.post("/llm_jobs", json={
        "job_type": "image_tag", "payload": {"m": "2"}, "user_id": test_user,
    })
    await test_app.post("/llm_jobs", json={
        "job_type": "intent_classify", "payload": {"m": "3"}, "user_id": test_user,
    })
...
async def test_list_llm_jobs_filter_user_id(test_app, test_user, test_db):
    ...
    await test_app.post("/llm_jobs", json={
        "job_type": "intent_classify", "payload": {"m": "2"}, "user_id": test_user,
    })
    await test_app.post("/llm_jobs", json={
        "job_type": "followup", "payload": {"m": "3"}, "user_id": other_user_id,
    })
...
async def test_list_llm_jobs_combined_filters(test_app, test_user, test_db):
    ...
    r1 = await test_app.post("/llm_jobs", json={
        "job_type": "image_tag", "payload": {"m": "1"}, "user_id": test_user,
    })
    await test_app.post("/llm_jobs", json={
        "job_type": "image_tag", "payload": {"m": "2"}, "user_id": test_user,
    ...
    await test_app.post("/llm_jobs", json={
        "job_type": "image_tag", "payload": {"m": "4"}, "user_id": other_user_id,
    })
    ...
    resp = await test_app.get("/llm_jobs", params={
        "user_id": test_user,
        "job_type": "image_tag",
        "status": "queued",
    })
